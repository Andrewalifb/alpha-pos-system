...
func main() {
	...
	branchSvc := service.NewPosStoreBranchServiceServer(branchRepo)
	companySvc := service.NewPosCompanyServiceServer(companyRepo)
	roleSvc := service.NewPosRoleServiceServer(roleRepo)
	storeSvc := service.NewPosStoreServiceServer(storeRepo)
